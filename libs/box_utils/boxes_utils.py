...
def get_horizen_minAreaRectangle(boxs, with_label=True):
    ...
    rpn_proposals_boxes_convert = tf.py_func(forward_convert,
                                             ...
                                             Tout=tf.float32)
    if with_label:
        rpn_proposals_boxes_convert = tf.reshape(rpn_proposals_boxes_convert, [-1, 9]) # [x1, y1, x2, y2, x3, y3, x4, y4, label]

        boxes_shape = tf.shape(rpn_proposals_boxes_convert)  # [N, 9]
