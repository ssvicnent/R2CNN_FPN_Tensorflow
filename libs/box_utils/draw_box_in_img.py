...
def draw_rotate_box_cv(img, boxes, labels, scores,img_name):
    img = img + np.array(cfgs.PIXEL_MEAN)
    boxes = boxes.astype(np.int64)
    labels = labels.astype(np.int32)
    img = np.array(img, np.float32)
    ...
    num_of_object = 0
    save_path = os.path.join(cfgs.INFERENCE_SAVE_PATH, cfgs.VERSION,cfgs.TXT)
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    with open(save_path+'/'+'res_{}.txt'.format(img_name.split('.jpg')[0]),'w') as f:
        for i, box in enumerate(boxes):
            ...
            if label != 0:
                ...
                category = LABEl_NAME_MAP[label]
                f.writelines(str(rect[0][0]))
                f.writelines(',')
                f.writelines(str(rect[0][1]))
                f.writelines(',')
                f.writelines(str(rect[1][0]))
                f.writelines(',')
                f.writelines(str(rect[1][1]))
                f.writelines(',')
                f.writelines(str(rect[2][0]))
                f.writelines(',')
                f.writelines(str(rect[2][1]))
                f.writelines(',')
                f.writelines(str(rect[3][0]))
                f.writelines(',')
                f.writelines(str(rect[3][1]))
                # f.writelines(',')
