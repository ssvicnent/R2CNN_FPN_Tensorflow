...
def anchor_target_layer(
    ...
    total_anchors = all_anchors.shape[0]  # anchor的数量
    ...
    argmax_overlaps = overlaps.argmax(axis=1) # 每个anchor都与gt_boxes求overlaps 取argmax代表anchor最可能是属于哪一个gt
    max_overlaps = overlaps[np.arange(len(inds_inside)), argmax_overlaps]
    gt_argmax_overlaps = overlaps.argmax(axis=0) # 每个gt_boxe都与anchors做overlaps
    ...
    num_fg = int(cfgs.RPN_MINIBATCH_SIZE * cfgs.RPN_POSITIVE_RATE)  # 128
    ...
    num_bg = cfgs.RPN_MINIBATCH_SIZE - np.sum(labels == 1) # 128
    if is_restrict_bg:
