...
def postprocess_rpn_proposals(rpn_bbox_pred, rpn_cls_prob, img_shape, anchors, is_training):
    ...
    if is_training:
        pre_nms_topN = cfgs.RPN_TOP_K_NMS_TRAIN  # 12000
        post_nms_topN = cfgs.RPN_MAXIMUM_PROPOSAL_TARIN # 2000
        nms_thresh = cfgs.RPN_NMS_IOU_THRESHOLD  # 0.7
    else:
        pre_nms_topN = cfgs.RPN_TOP_K_NMS_TEST # 10000
        post_nms_topN = cfgs.RPN_MAXIMUM_PROPOSAL_TEST # 300
        nms_thresh = cfgs.RPN_NMS_IOU_THRESHOLD  # 0.7
    ...
    # 2. clip to img boundaries  # 将超出图片框的anchors裁剪到图像边界
    decode_boxes = boxes_utils.clip_boxes_to_img_boundaries(decode_boxes=decode_boxes,
                                                            img_shape=img_shape)

    # 3. get top N to NMS
    if pre_nms_topN > 0:
        pre_nms_topN = tf.minimum(pre_nms_topN, tf.shape(decode_boxes)[0], name='avoid_unenough_boxes') # 避免框不够
        cls_prob, top_k_indices = tf.nn.top_k(cls_prob, k=pre_nms_topN) # 根据前景的分数来初次筛选
