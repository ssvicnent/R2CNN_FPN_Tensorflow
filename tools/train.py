...
def train():
    ...
    with tf.name_scope('get_batch'):
        img_name_batch, img_batch, gtboxes_and_label_batch, num_objects_batch = \
            next_batch(dataset_name=cfgs.DATASET_NAME,  # 'icdar2015', 'pascal', 'coco'
                       batch_size=cfgs.BATCH_SIZE,
                       shortside_len=cfgs.IMG_SHORT_SIDE_LEN,
                       is_training=True)

        gtboxes_and_label = tf.py_func(back_forward_convert,  # [x_c, y_c, w, h, theta, (label)]
                                       inp=[tf.squeeze(gtboxes_and_label_batch, 0)],
                                       Tout=tf.float32)
        gtboxes_and_label = tf.reshape(gtboxes_and_label, [-1, 6])

        gtboxes_and_label_AreaRectangle = get_horizen_minAreaRectangle(gtboxes_and_label) # [x_min, y_min, x_max, y_max, label]
    ...
    optimizer = tf.train.MomentumOptimizer(lr, momentum=cfgs.MOMENTUM) # define an optimizer
