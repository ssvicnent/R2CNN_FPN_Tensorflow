...
def make_dict_packle(_gtboxes_and_label, _fast_rcnn_decode_boxes, _fast_rcnn_score, _detection_category):
    ...
    for j, box in enumerate(_gtboxes_and_label):
        bbox_dict = {}
        bbox_dict['bbox'] = np.array(_gtboxes_and_label[j, :-1], np.float64) # coordinate
        bbox_dict['name'] = LABEl_NAME_MAP[int(_gtboxes_and_label[j, -1])]   # label
...
def eval(rboxes, gboxes, iou_th, use_07_metric, mode):
    ...
    fp = np.zeros(len(rbox_images))  # initialization
    tp = np.zeros(len(rbox_images))  # initialization
    box_num = 0
...
if __name__ == '__main__':
    ...
    for label in NAME_LABEL_MAP.keys():
        ...
        hboxes, gboxes = get_single_label_dict(predict_horizontal_dict, gtboxes_horizontal_dict, label)
        rboxes1, gboxes1 = get_single_label_dict(predict_rotate_dict, gtboxes_rotate_dict, label)

        rec, prec, ap, box_num = eval(hboxes, gboxes, 0.3, False, mode=0)
        rec1, prec1, ap1, box_num1 = eval(rboxes1, gboxes1, 0.3, False, mode=mode)
